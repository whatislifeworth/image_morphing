...
class Conv2DSymPadding(_Conv):
    ...
    def call(self, inputs):
        ...
        ph, pw = kh//2, kw//2
        inputs_pad = tf.pad(
            inputs, [[0, 0], [ph, ph], [pw, pw], [0, 0]], mode='symmetric')
        outputs = K.conv2d(
            inputs_pad,
            self.kernel,
...
class BayarConstraint(Constraint):
    ...
    def _initialize_mask(self, w):
        ...
        m = np.zeros([nb_rows, nb_cols, nb_inputs,
                     nb_outputs]).astype('float32')
...
def create_featex_vgg16_base(type=1):
    ...
    bname = 'b1'  # 32
    nb_filters = base
    x = CombinedConv2D(32 if type in [0, 1] else 16, activation='relu',
                       use_bias=False, padding='same', name=bname+'c1')(img_input)
    x = Conv2DSymPadding(nb_filters, (3, 3), activation='relu',
                         padding='same',  name=bname+'c2')(x)
    # block 2
    bname = 'b2'
    nb_filters = 2 * base  # 64
    x = Conv2DSymPadding(nb_filters, (3, 3), activation='relu',
                         padding='same',  name=bname+'c1')(x)
    x = Conv2DSymPadding(nb_filters, (3, 3), activation='relu',
                         padding='same',  name=bname+'c2')(x)
    # block 3
    bname = 'b3'
...
class NestedWindowAverageFeatExtrator(Layer):
    ...
    def __init__(self,
        ...
        assert output_mode in [
            '5d', '4d'], "ERROR: unkown output mode={}".format(output_mode)
    ...
    def call(self, x):
        ...
        if self.output_mode == '4d':
            ...
            raise (NotImplementedError,
                   "ERROR: unknown output_mode={}".format(self.output_mode))
...
def create_manTraNet_model(Featex, pool_size_list=[7, 15, 31], is_dynamic_shape=True, apply_normalization=True):
    ...
    rf = Conv2D(64, (1, 1),
                ...
                name='outlierTrans',
                padding='same')(rf)
    bf = BatchNormalization(axis=-1, name='bnorm',
                            center=False, scale=False)(rf)
    ...
    if (apply_normalization):
        ...
        sigma5d = Lambda(lambda t: K.expand_dims(
            t, axis=1), name='expTime')(sigma)
        devf5d = Lambda(lambda vs: K.abs(
            vs[0]/vs[1]), name='divStd')([devf5d, sigma5d])
    # convert back to 4d
    ...
    pred_out = Conv2D(1, (7, 7), padding='same',
                      activation='sigmoid', name='pred')(devf)
...
def load_pretrain_model_by_index(pretrain_index, model_dir):
    if (pretrain_index == 4):
        ...
        IMC_model_idx, freeze_featex, window_size_list = pretrain_index, False, [
            7, 15, 31, 63]
    single_gpu_model = create_model(
        IMC_model_idx, freeze_featex, window_size_list)
    weight_file = "{}/ManTraNet_Ptrain{}.h5".format(model_dir, pretrain_index)
    assert os.path.isfile(
        weight_file), "ERROR: fail to locate the pretrained weight file"
